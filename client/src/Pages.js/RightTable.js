...
import { setRightName, removeRightName } from "../Redux/slice/rightName";
import { setLeftName } from "../Redux/slice/leftName";

const RightTable = () => {
  const rightName = useSelector((state) => state.rightName.value) || [];
  ...
  const filteredNames = rightName.filter(name =>
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(setLeftName(selectedNames)); // Send selected items to the left table
    dispatch(removeRightName(selectedNames)); // Remove selected items from the right table
    setSelectedNames([]); // Reset selected items
  };
  ...
  const handleSubmitUsername = () => {
    if (username.trim() !== "") {
      dispatch(setRightName(username));
      setUsername("");
    }
  };
